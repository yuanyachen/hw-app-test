import time
from common.slide_swipe import swipe_up, swipe_left, ios_swipe_up
from common.pub_util import HanweiUtil
from selenium.common.exceptions import TimeoutException
from po.base_page import Base
from selenium.webdriver.common.by import By
from po.ios_page_entity import *
from selenium.common.exceptions import NoSuchElementException


class AndroidDriverUtil(Base):
    def __init__(self):
        super(AndroidDriverUtil, self).__init__("ANDROID")
        self.hwu = HanweiUtil()
        ...
        self.cpid = self.hwu.get_cpid(temporary_cpid)
        self.name = self.me_find_by_id('tv_mine_name').text
        self.customercareerlevel = self.hwu.get_customercareerlevel(self.cpid)
        self.hwu.logging_out.info('cpid=%s name=%s customercareerlevel=%s' % (self.cpid, self.name, self.customercareerlevel))
    ...
    def get_goods_info(self, is_collect=False, is_promotion=False):
        ...
        if 'white' != self.customercareerlevel and not is_promotion:  # 有身份的用户才会在页面上展示代理价 已减 立赚
            try:
                ...
                self.hwu.logging_out.error('有身份的用户获取页面详情页面失败！')
    ...
    def place_an_order(self, order_source='info', is_cash=False, pay_type='weixin', is_readdress=False, is_snap_up=False):
        ...
        for i in range(len(goods_names)):
            ...
            self.hwu.logging_out.info('商品%s购买了%s件, 合计%s元, 以减%s元' % (name, goods_number, price, discount_fee))

        if is_readdress:
            AndroidDriverUtil.address_test(self, is_redact=False)
        else:
            time.sleep(1)
        swipe_up(self.driver, 500, max=0.5)
        ...
        # self.hwu.logging_out.info('确认订单页面结算金额详情： %s' % order_pay_info)
    ...
    def seek_test(self, entrance, product_name, click=False):
        ...
        try:
            ...
            self.hwu.logging_out.info(ret)
            return ret

    def collect_test(self,collection_ele):
        '''
        ...
        collect_result = self.hwu.get_result_one(get_one_sql)
        if collect_result:
            effect_row = self.hwu.set_data(close_all_sql)
            self.hwu.logging_out.info('取消收藏%s件' % effect_row)

        self.me_find_by_id('ll_collect').click()  # 收藏/取消
        collect_result = self.hwu.get_result_one(get_one_sql)
        if collect_result:
            self.hwu.logging_out.info('收藏成功！')
        self.me_find_by_id('ll_back').click()
        self.me_find_by_id('iv_back').click()
        self.me_find_by_id('ll_main_tab_mine').click()
        collection_ele.click()

        collect_goods_info = AndroidDriverUtil.get_goods_info(self, True)
        self.hwu.logging_out.info(collect_goods_info)

        self.me_find_by_id('iv_delete').click()  # 取消收藏
        collect_result = self.hwu.get_result_one(get_one_sql)
        if not collect_result:
            self.hwu.logging_out.info('取消收藏成功！')
        self.me_find_by_id('iv_back').click()
    ...
    def close_car(self):
        ...
        result_all = self.hwu.get_result_all(get_all_sql)
        if len(result_all):
            effect_row = self.hwu.set_data(close_all_sql)
            self.hwu.logging_out.info('清除购物车%s件商品' % effect_row)
        self.me_find_by_id('ll_main_tab_shopping_car').click()
        t = self.me_find_by_id('tv_empty').text
        self.hwu.logging_out.info(t)
    ...
    def add_car(self, num):
        ...
        self.hwu.logging_out.info('价格:%s 库存:%s' % (sku_price, store_count))
        for i in range(num):
        ...
        self.hwu.logging_out.info('添加前:%s 添加后:%s 添加%s件' % (cart_count1, cart_count2, cart_count))
        self.me_find_by_id('ll_back').click()
        self.me_find_by_id('iv_back').click()
    ...
    def del_car_product(self, edit, check='all_checkBox'):
        ...
        if edit:
            ...
            for i in range(2, 0, -1):
                ...
                self.hwu.logging_out.info(message)
    ...
    def category_test(self):
        ...
        category_name_tuple = self.hwu.get_result_all(self, sql)
        i = 0
    ...
    def direct_order(self, is_category=False):
        ...
        goods_info = AndroidDriverUtil.get_goods_info(self)
        self.hwu.logging_out.info(goods_info)
        ...
        self.me_find_by_id('tv_confirm').click()
        yield
        self.hwu.logging_out.info('取消支付！')

    def get_serviers(self):
        ''''''
        swipe_up(self.driver, 1000,max=0.5)
        l = self.driver.find_elements_by_id('com.hanweiyx.hanwei:id/tv_name')
        servier_dict = {}
        servier_dict.setdefault('group_ele', l[0])  # 我的拼团
        servier_dict.setdefault('collection_ele', l[1])  # 我的收藏
        servier_dict.setdefault('address_ele', l[2])  # 我的地址
        try:
            self.driver.find_element_by_android_uiautomator("new UiSelector().text(\"游客足迹\")")
            servier_dict.setdefault('footprint_ele', l[3])  # 游客足迹
            servier_dict.setdefault('help_ele', l[4])  # 帮助中心
            servier_dict.setdefault('envelope_ele', l[5])  # 德分红包
            servier_dict.setdefault('results_ele', l[6])  # 我的业绩
        except NoSuchElementException as e:
            servier_dict.setdefault('help_ele', l[3])
            servier_dict.setdefault('envelope_ele', l[4])
        return servier_dict


    def address_test(self, address_ele, is_redact=True):
        '''
        1.进入我的页面/地址管理页面
    ...
    def car_order(self, num):
        ...
        time.sleep(2)
        try:
            self.hwu.logging_out.info(self.me_find_by_id('tv_empty').text)
            return False
        except Exception:
            if 3 == num:
            ...
            for i in range(len(product_names)):
                ...
                self.hwu.logging_out.info('商品%s 单价%s 购买%s件 ' % (product_name, goods_price, cart_count))
            while True:
                ...
                if '计算中' in total_price:
                    ...
                    self.hwu.logging_out.info('商品合计金额%s, 页面显示合计金额%s' % (total_price_s, total_price))
                    break
            ...
            self.me_find_by_id('go_pay').click()
            self.driver.find_element_by_id('android:id/button1').click()
            self.hwu.logging_out.info('共计%s %s种商品' % (total_price, pay_value))
            self.hwu.logging_out.info(message)
            return True
    ...
    def count_order(self):
        ...
        for status in status_s:
            sql = 'select count(*) from xquark_order where archive = 0 and status in ("%s") and buyer_id in (select id from xquark_user where cpId = "%s")' \
                  % (status[0], self.cpid)
            result = self.hwu.get_result_one(sql)
    ...
    def all_order_test(self):
        ...
        self.me_find_by_id('rl_look').click()
        ...
        # order_sum = self.hwu.get_result_one(sel_order_sum_sql)
        ...
        for i in range(1):
            ...
            try:
                ...
                self.hwu.logging_out.info(empty_text)
            except Exception:
                for i in range(2):
                    ...
                    order_info_sql = 'select xoi.product_name, xoi.market_price,xo.total_fee,xoi.amount,xo.status from xquark_order xo LEFT JOIN xquark_order_item xoi on(xo.id = xoi.order_id) where xo.order_no = "%s";' % order_no[
                                                                                                                                                                                                                             5:]
                    order_info = self.hwu.get_result_one(order_info_sql)
                    self.hwu.logging_out.info(order_info)
                    if not order_info:
                        self.hwu.logging_out.info('查询订单失败！')
                        break
                    self.hwu.logging_out.info('%s 商品名称:[%s:%s] 商品原价:[%s:%s] 支付金额:[%s:%s] 购买数量:[%s:%s] 订单状态:[%s:%s]'
    ...
    def help_test(self,help_ele):
        ...
        if '帮助中心' == self.me_find_by_id('tv_title').text:
            ...
            for i in directory_name_t:
                AndroidDriverUtil.examine_test(self, i[0], i[1])
        self.me_find_by_id('iv_back').click()
    ...
    def integral_test(self):
        ...
        point_total_tuple = self.hwu.get_result_one(point_total_sql)
        ...
        commission_total_tuple = self.hwu.get_result_one(commission_total_sql)
        ...
        ll_id = (('com.hanweiyx.hanwei:id/ll_score', '我的德分', (round(point_total_sum, 2), round(point_total_usable, 2))),
                 ('com.hanweiyx.hanwei:id/ll_integral', '我的收益',(round(commission_total_sum, 2), round(commission_total_usable, 2))))
        for i in ll_id:
            AndroidDriverUtil.examine_test(self, i[0], i[1], *i[2])
    ...
    def examine_test(self, directory_name_id, title, *args):
        ...
        if title == self.me_find_by_id('tv_title').text:
            if title in ('常见问题', '售后服务'):  # 如果是'常见问题', '售后服务' 则上拉
                ...
                if '我的德分' == title:
                    tv_point_total = self.me_find_by_id('tv_total').text
                    tv_point_usable = self.me_find_by_id('tv_usable').text
                else:
                    tv_point_total = self.me_find_by_id('tv_point_total').text
                    tv_point_usable = self.me_find_by_id('tv_point_usable').text

                for i in [(args[0], tv_point_total, '总'), (args[1], tv_point_usable, '可用')]:
                    if HanweiUtil.float_equal(float(i[0]), float(i[1])):
                        self.hwu.logging_out.info('%s的%s分正确, [数据库 %s: 页面 %s]' % (title, i[2], i[0], i[1]))
                    else:
                        self.hwu.logging_out.info('%s的%s分错误, [数据库 %s: 页面 %s]' % (title, i[2], i[0], i[1]))
            self.hwu.logging_out.info('%s页打开正常！' % title)
        else:
            self.hwu.logging_out.info('%s页打开出错！' % title)
        self.me_find_by_id('iv_back').click()  # 返回我的页面
    ...
    def promotion_test(self):
        ...
        promotion_info = self.hwu.get_result_one(sql)
        self.hwu.logging_out.info('页面商品名称: %s, 数据库: %s' % (page_for_details.get('goods_name'), promotion_info[2]))
        promotion_ciphertext = HanweiUtil.str_encode(promotion_info[0])
        self.hwu.logging_out.info(promotion_ciphertext)

        # 获取user_id,在获取改用户是否还可以购买该秒杀商品
        sql = 'select id from xquark_user where cpid = "%s";' % self.cpid
        user_id_ret = self.hwu.get_result_one(sql)
        ...
        if not promotion_ciphertext.get('code', None) or not user_id_encode_info.get('code', None):
            self.hwu.logging_out.info('key混淆加密失败！')
        else:
            promotion_id = promotion_ciphertext['msg']
            amount = self.hwu.get_redis_ret('%s:amount' % promotion_id)  # 剩余可购买件数
            limit_amount = self.hwu.get_redis_ret('%s:limitAmount' % promotion_id)  # 单个人购买限制数量
            r_result = self.hwu.get_redis_ret('%s:%s' % (user_id_encode_info['msg'], promotion_id))  # 用户已经购买的数量，空为没有购买
            lambda_fun = lambda: r_result if r_result else '0'
            purchase_num = lambda_fun()
            if int(amount) < 0 or int(limit_amount) <= int(purchase_num):
                self.hwu.logging_out.info('剩余可购买件数:%s 单个人购买限制数量:%s 用户已经购买的数量%s' % (amount, limit_amount, purchase_num))
                self.me_find_by_id('ll_back').click()
            else:
...
class IosDriverUtil(Base):
    def __init__(self):
        super(IosDriverUtil, self).__init__('IOS')
        self.hwu = HanweiUtil()

        # 进入我的页面，获取cpid name customercareerlevel身份信息 is_lightening是否点亮
        self.driver.find_element_by_accessibility_id('我的').click()
        ele_text_id = self.driver.find_element_by_ios_predicate('value BEGINSWITH "会员ID："').text
        self.cpid = self.hwu.get_cpid(ele_text_id)
        self.name = self.driver.find_element_by_xpath('//XCUIElementTypeStaticText[2]').text
        self.customercareerlevel = self.hwu.get_customercareerlevel(self.cpid)
        self.hwu.logging_out.info('cpid=%s name=%s customercareerlevel=%s' % (self.cpid, self.name, self.customercareerlevel))
    ...
    def help_test(self):
        ...
        self.driver.find_element_by_accessibility_id('帮助中心').click()
        for i in ('常见问题', '售后服务', '联系我们', '关于我们'):
    ...
    def address_test(self):
        ...
        self.driver.find_element_by_accessibility_id('地址管理').click()
        self.driver.find_element_by_accessibility_id('添加新地址').click()
        self.set_address()
    ...
    def set_address(self):
        ...
        :return:
        '''
        e2 = self.driver.find_element_by_accessibility_id('请输入收货人姓名')
        print(e2)
        l = self.driver.find_elements_by_class_name('XCUIElementTypeTextField')
        l[0].clear().send_keys('地址')
        l[1].clear().send_keys('13300000011')
        e = self.driver.find_elements_by_class_name('XCUIElementTypeTextView')
        e[0].clear().send_keys('详细地址')

        #
        # self.driver.find_element_by_accessibility_id('请选择').click()
        # self.driver.find_element_by_accessibility_id('完成').click()
        # self.driver.find_element_by_ios_predicate("type == 'XCUIElementTypeSwitch' and name == '设为默认地址'").click()
        # self.driver.find_element_by_accessibility_id('保存').click()

    def seek_porduct(self):
        self.driver.find_element_by_accessibility_id('首页').click()
        self.driver.find_element_by_ios_predicate("type == 'XCUIElementTypeTextField' AND value == '伊夫黎雪'").clear().send_keys('测试')
        self.ios_ele(**SearchPage.other_back_loc).click()

    def positioning_test(self):
        # l = self.ios_eles(**MyPage.total_loc)
        # print(l)
        str = self.driver.find_element_by_ios_predicate("type == 'XCUIElementTypeStaticText' and ' value MATCHES '"
                                                        "\d'").text
        print(str)
        # self.driver.find_element_by_xpath('//XCUIElementTypeStaticText[1]').click()

        # self.driver.find_elements_by_class_name('XCUIElementTypeTextField')[0].clear().send_keys('啊啊啊')
        # ios_swipe_up(self.driver,0.5,0.1,0.5)
        # l_ele = self.driver.find_elements_by_ios_predicate("type == 'XCUIElementTypeButton' and label != 'other back' and label != 'search trash' and label != 'search eye'")
        # print(l_ele)

        # self.driver.find_element_by_ios_predicate("type == 'XCUIElementTypeButton' and label == 'search trash'").click()
        # self.driver.find_element_by_accessibility_id('搜索').click()
        # self.driver.find_elements_by_accessibility_id('测试')[0].click()


if __name__ == '__main__':
    # hu = HanweiUtil()
    # name = hu.get_name()
    # self.hwu.logging_out.info(name)

    # sql = 'select name from xquark_category where taxonomy = "GOODS";'
    # product_name = hu.get_result_all(sql)
    # for i in product_name:
    #     self.hwu.logging_out.info(i[0])

    # tv_user_commission = str(round(float('0.1') - 0.01, 2))
    # self.hwu.logging_out.info('%s : %s' % (type(tv_user_commission), tv_user_commission))
    # self.hwu.logging_out.info(random.randint(100, 400))
    ...
    # self.hwu.logging_out.info(result_all)
    # self.hwu.logging_out.info(len(result_all))
    ...
    # self.hwu.logging_out.info(order_count)

    # exp1 = lambda: 1 if 1 == 1 else 0
    # self.hwu.logging_out.info(exp1())
